...
public class Calibrations {
	...
	public void spatializeWiiMotes2x(int x1, int x2, Wiimote wiimote,
			Wiimote wiimote2) {
	...
	public int[] offSetFunction(int x, int y) {
		...
		if (y > 511) {
			...
			y = (int) (oldY + (ur[1] - 512) / (ul[1] - 512) * shiftY
					* (oldY - 512) / YCF); // (Math.abs(ul[1] - ur[1])) );
		} else {
			// Bottom
			float myX = oldX - ll[0];
			shiftY = (myX / lrF[0]);
			...
			y = (int) (oldY - (512 - lr[1]) / (512 - ll[1]) * shiftY
					* (512 - oldY) / YCF);// (Math.abs(ll[1] - lr[1])));
		}
		...
		if (x > 511) {
			...
			x = (int) (oldX + (ll[0] + 512) / (ul[0] + 512) * shiftX
					* (oldX - 512) / XCF); // (Math.abs(ul[0] - ll[0])));
		} else {
			// Left
			...
			x = (int) (oldX - (512 - lr[0]) / (512 - ur[0]) * shiftX
					* (512 - oldX) / XCF); // (Math.abs(ur[0] - lr[0])));
		}
	...
	public int[][] getCalibPoints(javax.swing.JButton buttonState) {
		int[][] coords;
